(**
   ...
   Permet de charger et sauvegarder des images stockÃ©es dans des fichiers dans divers formats : png, jpg, gif, bmp, pgm, ppm.

   Necessite la suite logicielle ImageMagick.



   L'utilisation de ce module en mode interprete necessite l'appel Ã  l'interpreteur avec les options :
...
(**
   [dessiner_image t i j] dessine dans la fenetre graphique l'image representee par le tableau de pixels [t]. Le dessin est fait en i j en partant du coin inferieur gauche de la fenetre graphique.

   {b CU :} une fenetre graphique doit etre prealablement ouverte.
*)
val dessiner_image : Graphics.color array array -> int -> int -> unit
